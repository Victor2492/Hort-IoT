...
* **S07: Sensor de temperatura TMP36 (Parelles - 55 min)**
    * *Continguts:* Estudi del circuit integrat TMP36, patilla d'eixida i relació de proporcionalitat entre la tensió tramesa i els graus centígrads detectats. Aplicació de la fórmula de conversió analògica simplificada per a programació en blocs: $\text{temperatura } (^\circ\text{C}) = (\text{lectura\_ADC} \times 0.4882) - 50$ (on $0.4882 = \frac{500}{1023}$). Contextualització geogràfica real amb les temperatures esperades en el municipi de Gandia (Safor), oscil·lant de valors de 7°C (nit d'hivern) a 30°C (dia d'agost) per a verificació de rangs. Estudi de la instrucció de condició lògica `if/else` (blocs taronges de control) recolzada en operadors matemàtics de comparació ($<$, $\le$, $=$, $\ne$, $>$, $\ge$).
