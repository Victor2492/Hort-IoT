...
* **Explicació Teòrica:** El TMP36 és un sensor analògic de precisió que varia la seva tensió de sortida de manera lineal d'acord amb els canvis de temperatura ambiental. L'entorn TinkerCAD disposa del bloc d'entrada pur *leer sensor de temperatura en el pasillo Ax en unidades ºC*, el qual realitza automàticament les equacions de conversió elèctrica de l'element:
    $$\text{Temperatura }(^\circ\text{C}) = (\text{Lectura\_ADC} \times 0.4882) - 50$$
    Per a prendre decisions en robòtica utilitzem les estructures de condició lògica **`if / else`** (Si... aleshores... si no...). Aquestes avaluen si es compleix una sentència de comparació lògica matemàtica basant-se en operadors elementals ($<, \le, =, \ne, >, \ge$). Si la condició és certa, el controlador executa els blocs niats en la secció superior; si és falsa, salta directament a executar les instruccions de la secció `else`.
...
* **Explicació Teòrica:** El vent és un altre factor crític que afecta directament la distribució de l'aigua en els aspersors d'un camp agrícola. Un vent de caràcter fort desvia les gotes i evapora l'aigua abans de tocar el sòl, provocant un desbaratament inútil del recurs. Els anemòmetres són els sensors encarregats de mesurar la velocitat del vent. Al nostre laboratori virtual en TinkerCAD, simularem un anemòmetre calibrat emprant un potenciòmetre analògic adaptat mitjançant una relació d'escala directa programada sobre el codi:
    $$\text{Velocitat } (\text{km/h}) = \frac{\text{Valor\_ADC} \times 70}{1023}$$
...
| Criteri de Qualitat de la Pràctica | Nivell 5 (80% - 100%) [Excel·lent] | Nivell 4 (65% - 79%) [Aconseguit] | Nivell 3 (45% - 64%) [En procés] | Nivell 2 (25% - 44%) [Iniciat] | Nivell 1 (0% - 24%) [No assolit] |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Muntatge Físic i Ordre del Circuit** | El circuit és impecable, completament net i endreçat. Segueix estrictament la codificació corporativa de colors (Vermell=5V, Negre=GND). Components distribuïts de forma òptima sense encreuaments de cables que dificulten la visió del conjunt. | El circuit funciona correctament i mostra ordre general, tot i que presenta algun encreuament menor de cables o algun descuit aïllat en el codi de colors de les línies secundàries de senyal. | El circuit és funcional, però el disseny és confús o caòtic. Els cables estan desordenats o no es respecta el codi de colors obligatori de forma generalitzada a la protoboard. | Mostra desordre elèctric profund. Cables connectats de forma incorrecta o components mal disposats, malgrat funcionar o simular en condicions molt precises i parcials. | El circuit no és funcional, presenta errors crítics de connexió elèctrica o hi ha absència total de resistències limitadores de protecció, amb risc de cremar components reals. |
| **Lògica, Estructura i Resolució del Codi** | L'algorisme resol de manera brillant el repte. Aplica perfectament estructures complexes (histèresi, canvis de flanc o operadors de condició creuada) optimitzant el nombre de blocs. | El programa assoleix completament els objectius fixats a l'enunciat. La lògica és robusta i correcta, tot i que algun bloc podria optimitzar-se estructuralment. | El programa resol la part principal de la pràctica, però presenta errors en condicions de control secundàries, en l'anidament o en la configuració de retards temporals. | El programa mostra problemes seriosos en la seqüència lògica de blocs base i requereix suport d'ajuda constant per part del docent per poder simular. | El codi no s'executa, manca d'estructura lògica conceptual mínima o s'ha detectat un plagi directe copiat de fonts externes. |
| **Nomenclatura, Variables i Comentaris** | Utilitza nomenclatures completament descriptives per a elements. Incorpora comentaris de text rics (blocs grisos d'anotació) que detallen amb claredat el funcionament de cada seqüència. | Usa variables globals ben definides i estructurades. S'inclouen anotacions o comentaris textuals explicatius en els blocs de control més significatius del codi. | Declara variables a la memòria, però utilitza noms genèrics o poc descriptius (ex: `variable1`). Els comentaris de text són escassos o molt superficials. | Manca total d'anotacions descriptives al codi. Totes les variables creades estan mal anomenades o generen confusió en la lectura tècnica. | Absència absoluta de definició de variables, etiquetatges explicatius o blocs d'anotació per a la comprensió del programa informàtic. |
| **Monitorització i Depuració Sèrie** | Gestiona el terminal del Port Sèrie de manera impecable. Mostra dades amb el format textual exacte requerit i gestiona intervals temporals de sortida perfectes. | El monitor sèrie transmet la informació requerida per l'enunciat, tot i que el format final del text presenta desajustos menors d'espaiat o text. | Imprimeix paràmetres pel port sèrie, però s'observen pèrdues de dades, manca de salt de línia o retards de velocitat d'impressió mal configurats. | Mostra informació o valors numèrics de forma caòtica que no aporten utilitat real per a la tasca de depuració o interpretació tècnica. | No ha implementat el canal de comunicació pel Port Sèrie o la seva inserció genera bloquejos crítics que impedeixen la simulació de la pràctica. |